...
fn html_document(main_section_html: &str, frontmatter: &Frontmatter) -> String {
    ...
    let prismjs_dark_theme_css = include_bytes!("./resources/prism-one-dark.css");
    let prismjs_dark_theme_style_tag = format!(
        "<style>{}</style>",
        String::from_utf8_lossy(prismjs_dark_theme_css)
    );
    let prismjs_light_theme_css = include_bytes!("./resources/prism-one-light.css");
    let prismjs_light_theme_style_tag = format!(
        "<style>{}</style>",
    ...
    let theme_script = include_bytes!("./resources/theme.js");
    let theme_script_tag = format!("<script>{}</script>", String::from_utf8_lossy(theme_script));
...
<html lang="{language}">
  <head>
      ...
      {global_styles_tag}
      {prismjs_dark_theme_style_tag}
      {prismjs_light_theme_style_tag}"##
    );
      ...
      <div class="theme">
        ...
        >
          <path
            ...
            stroke-width="2"
            d="M12 16a4 4 0 1 0 0-8a4 4 0 0 0 0 8zm0-12h.01M20 12h.01M12 20h.01M4 12h.01m13.647-5.657h.01m-.01 11.314h.01m-11.324 0h.01m-.01-11.314h.01"
          />
        </svg>
        <label for="theme-toggle"><span class="screen-reader-text">Enable dark theme</span></label>
...
<html lang="en">
  <head>
      ...
      <link rel="apple-touch-icon" href="data:image/png;base64," >
      <meta name="theme-color" content="#032539" >
      <style>:root{--max-width-full:100%;--max-width-wrapper:38rem;--spacing-px:0.0625rem;--spacing-px-2:0.125rem;--spacing-px-3:0.1875rem;--spacing-0:0;--spacing-1:0.25rem;--spacing-2:0.5rem;--spacing-4:1rem;--spacing-5:1.25rem;--spacing-6:1.5rem;--spacing-7:1.75rem;--spacing-8:2rem;--spacing-12:3rem;--spacing-16:4rem;--font-family:"Helvetica Neue", helvetica, "Segoe UI", arial, freesans,
    sans-serif;--font-weight-normal:400;--font-weight-bold:700;--font-weight-black:900;--font-size-root:18px;--font-size-0:0.9rem;--font-size-1:1.125rem;--font-size-2:1.406rem;--font-size-4:2.197rem;--font-size-5:2.747rem;--font-size-6:3.433rem;--line-height-tight:1.3;--line-height-normal:1.5;--line-height-relaxed:1.75;--colour-green-white:hsl(69 21% 94%);--colour-green-white-tint-80:hsl(60 14% 99%);--colour-cocoa-brown:hsl(11 18% 12%);--colour-cocoa-brown-shade-30:hsl(9 16% 8%);--colour-elm:hsl(193 67% 34%);--colour-elm-tint-90:hsl(195 35% 93%);--colour-saffron:hsl(44 94% 58%);--colour-saffron-tint-10:hsl(44 95% 62%);--colour-saffron-shade-10:hsl(44 75% 52%);--colour-tangaroa:hsl(202 47% 21%);--colour-tangaroa-tint-10:hsl(202 29% 29%);--colour-zest:hsl(34 80% 49%);--colour-heading:var(--colour-tangaroa);--colour-surface:var(--colour-green-white-tint-80);--colour-text:var(--colour-cocoa-brown);--colour-text-background-accent:var(--colour-elm-tint-90);--colour-text-alt:var(--colour-tangaroa-tint-10)}.dark-theme{--colour-heading:var(--colour-zest);--colour-surface:var(--colour-cocoa-brown-shade-30);--colour-text:var(--colour-green-white);--colour-text-background-accent:var(--colour-tangaroa);--colour-text-alt:var(--colour-saffron-shade-10)}*,:after,:before{box-sizing:border-box}*{margin:0}html{-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;scroll-behavior:smooth}@media (prefers-reduced-motion:reduce){html{scroll-behavior:auto}}body{display:flex;flex-direction:column;font:1.125rem/1.5"Helvetica Neue",helvetica,"Segoe UI",arial,freesans,sans-serif;font:var(--font-size-1)/var(--line-height-normal) var(--font-family);color:hsl(11 18% 12%);color:var(--colour-text);text-rendering:optimizelegibility;background-color:hsl(60 14% 99%);background-color:var(--colour-surface);transition:color .8s ease}@media (prefers-reduced-motion:reduce){body{transition:color 2s ease}}header,main{max-width:38rem;max-width:var(--max-width-wrapper);margin-top:2rem;margin-top:var(--spacing-8);margin-inline:auto}header{display:flex;width:100%}main{margin-bottom:4rem;margin-bottom:var(--spacing-16)}h1,h2{font-size:2.747rem;font-size:var(--font-size-5);color:hsl(202 47% 21%);color:var(--colour-heading)}h2{font-size:2.197rem;font-size:var(--font-size-4)}h3{font-size:var(--font-size-3)}h4{font-size:1.406rem;font-size:var(--font-size-2)}h1,h2,h3,h4,h5,h6{margin:3rem 0 1.5rem;margin:var(--spacing-12) var(--spacing-0) var(--spacing-6);line-height:1.3;line-height:var(--line-height-tight)}h2,h3,h4,h5,h6{font-weight:700;font-weight:var(--font-weight-bold)}p{line-height:1.75;line-height:var(--line-height-relaxed);margin:0 0 1rem;margin:var(--spacing-0) var(--spacing-0) var(--spacing-4);padding:0;padding:var(--spacing-0)}p code{background-color:hsl(195 35% 93%);background-color:var(--colour-text-background-accent);border-radius:.125rem;border-radius:var(--spacing-px-2);padding:.0625rem .25rem;padding:var(--spacing-px) var(--spacing-1);-webkit-box-decoration-break:clone;box-decoration-break:clone;margin-bottom:1rem;margin-bottom:var(--spacing-4)}ol,ul{margin-inline:0;margin-inline:var(--spacing-0);margin-bottom:2rem;margin-bottom:var(--spacing-8);list-style-position:inside}:is(ol,ul) li{margin-bottom:1rem;margin-bottom:var(--spacing-4)}li>ul{margin-left:2rem;margin-left:var(--spacing-8)}li:last-child{margin-bottom:0;margin-bottom:var(--spacing-0)}a{color:hsl(202 29% 29%);color:var(--colour-text-alt);text-underline-offset:.1875rem;text-underline-offset:var(--spacing-px-3)}a:focus,a:hover{text-decoration:none}img,pre{max-width:100%}pre{margin-top:3rem;margin-top:var(--spacing-12);margin-bottom:4rem;margin-bottom:var(--spacing-16);width:100%;width:var(--max-width-full);max-width:var(--max-width-full);overflow-x:auto}img{margin:2rem 0 1.5rem;margin:var(--spacing-8)0 var(--spacing-6)}.heading-anchor{display:none}h2:hover .heading-anchor{display:inline}.theme{display:flex;margin-left:auto;gap:.5rem;gap:var(--spacing-2);align-items:center}.theme .moon{opacity:40%}.dark-theme .theme .moon,.theme .sun{opacity:100%}.dark-theme .theme .sun{opacity:40%}.theme-toggle{-webkit-appearance:none;position:relative;width:3rem;width:var(--spacing-12);height:1.75rem;height:var(--spacing-7);border-radius:1.5rem;border-radius:var(--spacing-6);background-color:hsl(195 35% 93%);background-color:var(--colour-text-background-accent);transition:background .3s;border:.0625rem solid hsl(202 29% 29%);border:var(--spacing-px) solid var(--colour-text-alt);outline:0;cursor:pointer}.theme-toggle::after{content:"";position:absolute;top:50%;left:30%;transform:translate(-50%,-50%);border-radius:50%;height:1.25rem;height:var(--spacing-5);width:1.25rem;width:var(--spacing-5);background-color:hsl(202 47% 21%);background-color:var(--colour-heading);transition:left .3s}.theme-toggle:checked::after{left:70%}.screen-reader-text{border:0;clip:rect(1px,1px,1px,1px);-webkit-clip-path:inset(50%);clip-path:inset(50%);height:1px;margin:-1px;width:1px;overflow:hidden;position:absolute!important;word-wrap:normal!important}</style>
      <style>/**
 * One Dark theme for prism.js
 * Based on Atom's One Dark theme: https://github.com/atom/atom/tree/master/packages/one-dark-syntax
 */
code[class*=language-],pre[class*=language-]{background:#282c34;color:#abb2bf;text-shadow:0 1px rgba(0,0,0,.3);font-family:"Fira Code","Fira Mono",Menlo,Consolas,"DejaVu Sans Mono",monospace;direction:ltr;text-align:left;white-space:pre;word-spacing:normal;word-break:normal;line-height:1.5;-moz-tab-size:2;-o-tab-size:2;tab-size:2;-webkit-hyphens:none;hyphens:none}code[class*=language-] ::-moz-selection,code[class*=language-]::-moz-selection,pre[class*=language-] ::-moz-selection{background:#3e4451;color:inherit;text-shadow:none}code[class*=language-] ::selection,code[class*=language-]::selection,pre[class*=language-] ::selection{background:#3e4451;color:inherit;text-shadow:none}pre[class*=language-]{padding:1em;margin:.5em 0;overflow:auto;border-radius:.3em}:not(pre)>code[class*=language-]{padding:.2em .3em;border-radius:.3em;white-space:normal}@media print{code[class*=language-],pre[class*=language-]{text-shadow:none}}.token.cdata,.token.comment,.token.prolog{color:#5c6370}.token.attr-value>.token.punctuation.attr-equals,.token.doctype,.token.entity,.token.punctuation,.token.special-attr>.token.attr-value>.token.value.css{color:#abb2bf}.token.atrule,.token.attr-name,.token.boolean,.token.class-name,.token.constant,.token.number{color:#d19a66}.token.keyword{color:#c678dd}.language-css .token.selector,.token.deleted,.token.important,.token.property,.token.symbol,.token.tag{color:#e06c75}.language-css .token.url>.token.string.url,.token.attr-value,.token.attr-value>.token.punctuation,.token.builtin,.token.char,.token.inserted,.token.regex,.token.selector,.token.string{color:#98c379}.token.function,.token.operator,.token.variable{color:#61afef}.language-css .token.property{color:#abb2bf}.language-css .token.function,.language-css .token.url>.token.function,.token.url{color:#56b6c2}.language-css .token.atrule .token.rule,.language-css .token.important,.language-javascript .token.operator{color:#c678dd}.language-javascript .token.template-string>.token.interpolation>.token.interpolation-punctuation.punctuation{color:#be5046}.language-json .token.operator,.language-markdown .token.url,.language-markdown .token.url-reference.url>.token.string,.language-markdown .token.url>.token.operator{color:#abb2bf}.language-json .token.null.keyword{color:#d19a66}.language-markdown .token.url>.token.content{color:#61afef}.language-markdown .token.url-reference.url,.language-markdown .token.url>.token.url{color:#56b6c2}.language-markdown .token.blockquote.punctuation,.language-markdown .token.hr.punctuation{color:#5c6370;font-style:italic}.language-markdown .token.code-snippet{color:#98c379}.language-markdown .token.bold .token.content{color:#d19a66}.language-markdown .token.italic .token.content{color:#c678dd}.language-markdown .token.list.punctuation,.language-markdown .token.strike .token.content,.language-markdown .token.strike .token.punctuation,.language-markdown .token.title.important>.token.punctuation,.rainbow-braces .token.token.punctuation.brace-level-1,.rainbow-braces .token.token.punctuation.brace-level-5,.rainbow-braces .token.token.punctuation.brace-level-9{color:#e06c75}.token.bold{font-weight:700}.token.comment,.token.italic{font-style:italic}.token.entity{cursor:help}.token.namespace{opacity:.8}.token.token.cr:before,.token.token.lf:before,.token.token.space:before,.token.token.tab:not(:empty):before{color:rgba(171,178,191,.15);text-shadow:none}div.code-toolbar>.toolbar.toolbar>.toolbar-item{margin-right:.4em}div.code-toolbar>.toolbar.toolbar>.toolbar-item>a,div.code-toolbar>.toolbar.toolbar>.toolbar-item>button,div.code-toolbar>.toolbar.toolbar>.toolbar-item>span{background:#3a3f4b;color:#828997;padding:.1em .4em;border-radius:.3em}div.code-toolbar>.toolbar.toolbar>.toolbar-item>a:focus,div.code-toolbar>.toolbar.toolbar>.toolbar-item>a:hover,div.code-toolbar>.toolbar.toolbar>.toolbar-item>button:focus,div.code-toolbar>.toolbar.toolbar>.toolbar-item>button:hover,div.code-toolbar>.toolbar.toolbar>.toolbar-item>span:focus,div.code-toolbar>.toolbar.toolbar>.toolbar-item>span:hover{background:#3e4451;color:#abb2bf}.line-highlight.line-highlight{background:rgba(153,187,255,.04)}.line-highlight.line-highlight:before,.line-highlight.line-highlight[data-end]:after{background:#3a3f4b;color:#abb2bf;padding:.1em .6em;border-radius:.3em;box-shadow:0 2px 0 0 rgba(0,0,0,.2)}pre[id].linkable-line-numbers.linkable-line-numbers span.line-numbers-rows>span:hover:before{background-color:rgba(153,187,255,.04)}.command-line .command-line-prompt,.line-numbers.line-numbers .line-numbers-rows{border-right-color:rgba(171,178,191,.15)}.command-line .command-line-prompt>span:before,.line-numbers .line-numbers-rows>span:before{color:#636d83}.rainbow-braces .token.token.punctuation.brace-level-10,.rainbow-braces .token.token.punctuation.brace-level-2,.rainbow-braces .token.token.punctuation.brace-level-6{color:#98c379}.rainbow-braces .token.token.punctuation.brace-level-11,.rainbow-braces .token.token.punctuation.brace-level-3,.rainbow-braces .token.token.punctuation.brace-level-7{color:#61afef}.rainbow-braces .token.token.punctuation.brace-level-12,.rainbow-braces .token.token.punctuation.brace-level-4,.rainbow-braces .token.token.punctuation.brace-level-8{color:#c678dd}pre.diff-highlight>code .token.token.deleted:not(.prefix),pre>code.diff-highlight .token.token.deleted:not(.prefix){background-color:rgba(255,82,102,.15)}pre.diff-highlight>code .token.token.deleted:not(.prefix) ::-moz-selection,pre.diff-highlight>code .token.token.deleted:not(.prefix)::-moz-selection,pre>code.diff-highlight .token.token.deleted:not(.prefix) ::-moz-selection,pre>code.diff-highlight .token.token.deleted:not(.prefix)::-moz-selection{background-color:rgba(251,86,105,.25)}pre.diff-highlight>code .token.token.deleted:not(.prefix) ::selection,pre.diff-highlight>code .token.token.deleted:not(.prefix)::selection,pre>code.diff-highlight .token.token.deleted:not(.prefix) ::selection,pre>code.diff-highlight .token.token.deleted:not(.prefix)::selection{background-color:rgba(251,86,105,.25)}pre.diff-highlight>code .token.token.inserted:not(.prefix),pre>code.diff-highlight .token.token.inserted:not(.prefix){background-color:rgba(26,255,91,.15)}pre.diff-highlight>code .token.token.inserted:not(.prefix) ::-moz-selection,pre.diff-highlight>code .token.token.inserted:not(.prefix)::-moz-selection,pre>code.diff-highlight .token.token.inserted:not(.prefix) ::-moz-selection,pre>code.diff-highlight .token.token.inserted:not(.prefix)::-moz-selection{background-color:rgba(56,224,98,.25)}pre.diff-highlight>code .token.token.inserted:not(.prefix) ::selection,pre.diff-highlight>code .token.token.inserted:not(.prefix)::selection,pre>code.diff-highlight .token.token.inserted:not(.prefix) ::selection,pre>code.diff-highlight .token.token.inserted:not(.prefix)::selection{background-color:rgba(56,224,98,.25)}.prism-previewer-gradient.prism-previewer-gradient div,.prism-previewer.prism-previewer:before{border-color:#262931}.prism-previewer-color.prism-previewer-color:before,.prism-previewer-easing.prism-previewer-easing:before,.prism-previewer-gradient.prism-previewer-gradient div{border-radius:.3em}.prism-previewer.prism-previewer:after{border-top-color:#262931}.prism-previewer-flipped.prism-previewer-flipped.after{border-bottom-color:#262931}.prism-previewer-angle.prism-previewer-angle:before,.prism-previewer-easing.prism-previewer-easing,.prism-previewer-time.prism-previewer-time:before{background:#31363f}.prism-previewer-angle.prism-previewer-angle circle,.prism-previewer-time.prism-previewer-time circle{stroke:#abb2bf;stroke-opacity:1}.prism-previewer-easing.prism-previewer-easing circle,.prism-previewer-easing.prism-previewer-easing line,.prism-previewer-easing.prism-previewer-easing path{stroke:#abb2bf}.prism-previewer-easing.prism-previewer-easing circle{fill:transparent}
</style>
      <style>/**
...
<p>This is a test.</p>
    ...
    <script>(()=>{var e=document.querySelector(".theme-toggle");window.matchMedia("(prefers-color-scheme: dark)").matches?(document.styleSheets[2].disabled=!0,document.querySelector("body").classList.add("dark-theme"),e.checked=!0):document.styleSheets[1].disabled=!0;e.addEventListener("change",()=>{document.styleSheets[1].disabled=!document.styleSheets[1].disabled,document.styleSheets[2].disabled=!document.styleSheets[2].disabled,document.querySelector("body").classList.toggle("dark-theme")});})();
</script>
